...
double meval_var(const char* input_string, MEvalVarArr variables, struct MEvalError* error) {
    ...
    bool error_occured = false;
    const char* error_string = NULL;
    gen_lex_tokens(input_string, strlen(input_string), true, &tokens, &lex_tokens_count, &error_occured);
    if (lex_tokens_count == 0) {
        error->type = MEVAL_LEX_ERROR;
        error->char_index = 0;
    ...
    if (error_occured) {
        ...
        fprintf(stderr, "Error occured ... But cannot find the errored token\n");
        abort();
        // Error occured ... But cannot find it?????
    }
    enum RPN_ERROR rpn_error = RPNE_NONE;
    LexToken* rpn_tokens = NULL;
    uint32_t rpn_tokens_count = 0;
    gen_reverse_polish_notation(tokens, lex_tokens_count, true, &rpn_tokens, &rpn_tokens_count, &rpn_error);
    for (size_t i=0; i < rpn_tokens_count; i++) {
        printf("RPN Token: ");
        print_token(rpn_tokens[i]);
    }
